...
export default class BoatOwnersList extends Component {
  ...
  btnClick()
  {
    this.props.navigation.navigate("Committee");
  }
  ...
  {
    fetch('http://zeveninfoway.com/sagarputra/index.php/webservice/boatOwnerList',
    {
        method: 'POST',
        headers:
            {
                ...
                renderRow={(rowData) =>{
                    return(
                      <TouchableOpacity style={[styles.viewWrap,{flex:1, flexDirection: 'row'}]}
                                        ...
                                        >
                        <Image source = { rowData.boat_ownes_pic!="" ? { uri:rowData.boat_ownes_pic} : require('../images/default.jpg')}
                                style={[styles.imageViewContainer,{marginRight:5}]} />
    
                        <View style={{flex:1, flexDirection: 'column'}}>
                            <TextLable text={rowData.boat_ownes_name}
                                      color={AppStyles.colour.News_ID_font} 
                                      />
                            <TextLable text={rowData.boat_name +" - "+ rowData.address} 
